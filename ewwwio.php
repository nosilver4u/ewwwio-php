...
class EWWWIO {
	...
	function __construct( $api_key = '' ) {
		...
		if ( $this->debug ) {
			if ( ! extension_loaded( 'suhosin' ) && function_exists( 'get_current_user' ) ) {
				$this->debug_log .= get_current_user() . '<br>';
			}

			$this->debug_log .= 'EWWW IO version: ' . EWWW_IMAGE_OPTIMIZER_VERSION . "\n";

			if ( defined( 'PHP_VERSION_ID' ) ) {
				$this->debug_log .= 'PHP version: ' . PHP_VERSION_ID . "\n";
			}
		}
	}
	...
	*/
	...
	function optimize( $file ) {
		...
		if ( ini_get( 'max_execution_time' ) < 90 && $this->stl_check() ) {
			set_time_limit( 0 );
		}
		...
		switch ( $type ) {
			...
			case 'application/pdf':
				if ( $this->pdf_level ) {
					list( $file, $converted, $result, $new_size ) = $this->cloud_optimizer( $file, $type );
				}
				break;
			default:
